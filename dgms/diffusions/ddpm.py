...
# TODO: DDPM.negative_elbo(...) should return the negative ELBO of equation (14) by Ho
# TODO: DDPM.sample(shape) should implement Algorithm 2 in the paper by Ho et al.
...
class DDPM(nn.Module):
    ...
    def negative_elbo(self, x):
        ...
        t = torch.randint(1, self.T + 1, (batch_size,)).to(device)

        # Compute alpha_cumprod_t and alpha_cumprod_t_minus_1
        alpha_cumprod_t = self.alpha_cumprod[t - 1].view(-1, 1)
        alpha_cumprod_t_minus_1 = self.alpha_cumprod[t - 2].view(-1, 1) if t.min() > 1 else torch.ones_like(alpha_cumprod_t)
        ...
        mse_loss = F.mse_loss(epsilon_theta, epsilon, reduction='none').mean(dim=1)
        kl_loss = 0.5 * (1 - alpha_cumprod_t_minus_1 / alpha_cumprod_t).sum(dim=1)

        neg_elbo = mse_loss + kl_loss
    ...
    def sample(self, shape):
        ...
        for t in range(self.T-1, -1, -1):
            t_tensor = torch.full((shape[0], 1), t, device=self.alpha.device, dtype=torch.float32) / self.T
            ...
            if t > 0:
                alpha_cumprod_t_minus_1 = self.alpha_cumprod[t - 1]
                mean = (1 / torch.sqrt(alpha_t)) * (x_t - (beta_t / torch.sqrt(1 - alpha_cumprod_t)) * epsilon_theta)
                variance = beta_t * (1 - alpha_cumprod_t_minus_1) / (1 - alpha_cumprod_t)
